...
public class Proj7Fkhan42
{
 ...
 /*This is for the extra credit */
  public void doWhere(Scanner sc)
  {
	  int val1=0;
	  	...
	  	if(val1<1 || val1>= air.length)//checks withing range dont forget the 0 it counts for that 
		 {
			 System.out.println ("doWhere airport does not exist");
			 return;
		 }
	  	
	  	System.out.println ("Performing the Where Command with " + (val1) );
	  	
	  	whereToHelper(val1); 
	  	
  }
  
  void whereToHelper (int x) //Logic provided by Prof.Troy
  {
	
	  air[0]= new Airport(0);
	  
	  whereTo(x);
	  System.out.println("The airports that can be reached from airport " + x+ " are:");
	  air[0].printList();
  }
  ...
  public void whereTo(int a)//Logic provided by Prof. Troy
  {
	  int b;
	  for(b = air[a].returnNotOnList(air) ;b!=-999; b = air[a].returnNotOnList(air))
	  {
		  
		  if(air[0].isNotMemberOfL(b)== true)
		  {
