...
public class Airport 
{
	...
	private int airportNum;//essentially the index position
	  ...
	  public void printList()
	  {
		  ...
		  while(traverse!=null)
		  {
			  System.out.print(traverse.airport + " ");
	...
	public boolean isNotMemberOfL(int b)
	  {
		  //int i= 0;
		  Node traverse = head;
		  while(traverse!=null)
		  {
			  if (b == traverse.airport)
				  return false;
	...
	public int returnNotOnList(Airport [] air)
	{
		Node traverse = head ; 
		Node traverseOnList = air[0].head; 
		
		while(traverse !=null)
		{
			while(traverseOnList!= null&& traverse !=null)
			{
				if(traverse.airport == traverseOnList.airport)
				{
					traverseOnList = air[0].head;
					traverse = traverse.next;
				}
				else
					traverseOnList= traverseOnList.next;
				
			}
			
				if(traverseOnList == null && traverse !=null)
	...
	{//This is watch actully not necessay, I could just use the push method 
		Node tempNode = new Node(connectAirport);
		head = tempNode; 
	}
	
	public void link (String fileName)//This is watch actully not necessay, I could just use the push method 
	{
